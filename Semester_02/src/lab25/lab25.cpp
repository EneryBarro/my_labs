...
private:
    ...
    int year;
public:
    Zoo(int d = 0, int m = 0, int y = 0) : day(d), month(m), year(y)
    {
    }
    friend ostream& operator<< (ostream& out, const Zoo& Zoo);
    friend istream& operator>> (std::istream& in, Zoo& zoo);
...
istream& operator>> (istream& in, Zoo& zoo)
{
    in >> zoo.day;
    in >> zoo.month;
    in >> zoo.year;
...
ostream& operator<< (ostream& out, const Zoo& zoo)
    {
        out << "Date: " << zoo.day << ". " << zoo.month << ". " << zoo.year << endl;
